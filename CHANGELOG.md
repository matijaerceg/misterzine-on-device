...
- Show the main-menu entry only after Setup has run. The database no longer ships `MisterZine.mgl`, so a card whose files came from Downloader alone, such as one installed through MiSTer Companion, showed an entry that reloaded the menu core over a blank picture and read as a broken install. Setup already writes the entry, and the watcher now writes it when it starts as well, so an existing card keeps its entry after the update run removes the copy the database used to track. Troubleshooting says that a missing entry means Setup has not run, whichever installer put the files there.
- Credits: Alec Peden, Arthur Shingler, BaronAloha, Bob, Charles Dreiss, Christoph Helms, Christopher K Sands, fandenivoldsk, it290, J, kobiyashi, Mathieu HERVAIS, Nate, Nick Winn, phil harris, Random Hajile, Renaud Schweingruber, Tim Hendrix and Tom Quarterman join the Patreon supporters.
- Embedded catalogue snapshot refreshed (1,302 rows, 2026-09-24).

## v1.1.3 — 2026-09-23

- Mark a game in the list when its ROMs stop it, before anyone opens Details. The ROM check used to run only for the version selected in Details and again on Start, so a game with a broken ROM set showed the ordinary `+` card mark until the player got that far; ac3 asked on Discord for a mark in the list and a filter for games with ROM problems. After every card scan a background pass now runs the same check, one file at a time off the drawing loop, over every arcade game on the card and every version the card offers, and stores each answer where Details already reads it. The list shows `!` in place of the card mark when the version Start would launch has a problem: in the warning colour when MiSTer could not load it, greyed when it is a version mismatch the game may still run with. The mark follows the version chosen in Details. Alternates do not mark the list, since most cards hold a few clones with problems and the mark would mean nothing; Details prefixes such a version with `!` in the version picker and says "ROM problems in 2 of 12 versions" under the main line instead.
- Add Filters -> ROM check, after On the card: everything, problem in launch version, problem in any version, or no problems found, each with its count, and a line saying how far the background pass has got while it runs. The card report's per-game filter reasons name it like every other rule.
- The pass starts on a later tick than the scan that asked for it, rests 3 ms between files and repaints at most four times a second while answers change what the list shows. Measured on a MiSTer Pi with 3,163 files over eleven relaunches with injected presses: the pass takes 36-38 s, and scrolling during it paints in 8.7 ms on average against 8 ms afterwards, with the same slowest frame. The one 150-200 ms frame seen right after a card scan happens with the pass switched off as well: it is the repaint of the newly scanned row set, unchanged by this release. A single check that takes over 80 ms is logged (a few Atari and Universal sets with many inline parts take up to 800 ms).
- Credits: Wasabi joins the early adopters.
- Embedded catalogue snapshot refreshed (1,277 rows, 2026-09-23).

## v1.1.2 — 2026-09-22

- Offer every set of a game the card can play as a version of it, not only the ones in the `_alternatives` folders that run on the game's own core. The first card report, from a DE10, listed 47 files MiSTer's menu runs that MisterZine never offered: sets for another core that is on the card (Pleiads' own alternatives run on the Pleiads core while the catalogue plays the game on Phoenix; Shinobi, SDI and Tetris on System 16B beside the System 16A core; Lunar Lander on its own core beside Asteroids), and sets of the game's own core kept outside the `_alternatives` folders (Jungle Hunt beside Jungle King). A game that runs on the card now offers such a file when the file's own core is in `_Arcade/cores` too, labelled with that core in the version picker, and drops it the moment the core leaves. Belonging is strict and never decided by ROM zip names, because the catalogue's MAME families span different titles (Sprint 1 and 2, River Patrol and Silver Land, Gradius and Nemesis): the game whose set name it is owns a file outright, a parent or family claim counts only when every claimant is the same release (Black Heart in two implementations, Asteroids Deluxe in two colours), a claim spanning releases offers the file nowhere and the card report says so, and a family sibling never stands in for the exact game when that one cannot run. A game counts as running when the list shows it on the card, core and main MRA both, so a game whose main MRA is missing shows greyed and a copy that runs stands in for it. A file already offered by one implementation reaches the game's other implementations on other cores (Turtles in Time, Caveman Ninja, Crude Buster, Edward Randy, Vapor Trail between Coin-Op and the other databases). BIOS files are never versions. Two files of one name show their folder, and Details keeps the chosen version by its file, so a rescan that reorders the list never switches the choice and a version that has gone falls back to the main one. A stand-in that gives way hands its favorite and remembered version to the game that now offers its file, rather than to whichever catalogue row covers the set name on paper. On the DE10 the card gained 70 versions, each under its own game, and a Pleiads set chosen in Details launched on the Pleiads core.
- Add Send a report to Options -> Troubleshooting, so a game missing from a player's list can be looked into from a short code instead of a round of questions. It says what goes into the report and, on A, writes a plain-text description of the card: the app version, board, INI summary and display; the list's order and the filters in force; every local game and the filter rule hiding it, if any, taken from the same test the list uses; every game file that is not in the list with the reason (a folder the scan does not read, an unreadable file, a BIOS file, a file without a set name, a game whose core is not on the card, a set whose family spans several games); the card scan's totals; the card's layout (top level, the `_Arcade` tree with its MRA counts, every file in `_Arcade/cores`, the other core folders' counts, the arcade ROM folder MiSTer picks with its zip counts, and MisterZine's own folder); `settings.json`; and the last 300 log lines, with the query part of any web address and the value of anything named like a key removed. It holds no passwords, Wi-Fi details or Downloader database addresses and is capped at 256 KB. It is always saved on the card as `misterzine/report.txt`, then uploaded to `api.misterzine.fyi`, which answers with a four-character code drawn only from characters nothing else can be mistaken for (`34679ACEFHJKMNPRTWXY`); offline, or when the service turns it away, the screen says why and points at the card copy. Reports are stored privately, readable only by the developer, keep nothing about who sent them, and are deleted after 30 days. The site's privacy page describes it.
- List a stand-in right under the greyed catalogue row it replaces in every order but Favorites and Recents. A stand-in has no dates of its own, so in Build date, the order MisterZine opens in, it sank to the end of some 1,280 rows while the greyed row sat near the top, rather than beside it as the 1.1.1 notes said. The letter, year and manufacturer headers and the L/R jumps treat the stand-in as part of that row's group, so a stand-in named differently opens no header of its own and its missing date is not a "Date unknown" stop. Thanks to Mclaneinc for reporting it.
- Look inside the zips for the missing-ROM warning, the way MiSTer loads them, instead of only checking that each archive exists: a zip that lacked a file, or held another version of it, passed and launched into a black screen, a clone whose own zip was missing passed on its parent's, and a folder named like a zip counted although MiSTer cannot read one. Each part the MRA names is looked for in its zips in order, by checksum and then by name, and the warning names what to fix: Missing game ROM, Incomplete ROM and Unreadable ROM stop Start; Wrong ROM version only warns, and stops Start only when the ROM MiSTer assembles really fails its MRA's md5, rebuilt the way Main builds it (Galaxian (New Invasion) from HBMame plays although its part checksums are stale). Index-0 sections follow Main's order. A `mame` folder at the top of the SD card is reported for what it does: Main takes it before `games/mame` and then looks for every zip under `/mame` at the filesystem root, so no arcade game can load; the check says to move its contents into `games/mame`. Details no longer waits on the card while drawing: the check keeps its parsed MRAs and zip directories until the files change and repaints when an answer lands, and Start always checks afresh.
- Fill a 1440p or 4K screen instead of a stretched band across its middle. MiSTer sends any mode wider than 2048 pixels with HDMI pixel repetition, so 2560x1440 goes out as 1280x1440 and the display doubles every pixel, which MisterZine's framebuffer window did not know about. Where the hardware reports repetition, MisterZine now asks for a framebuffer twice the canvas height and writes each row twice; the canvas, text size and safe zone are unchanged. Repetition is read from the frame the scaler drives, with one probe only when the video mode could repeat, since a 720p display and a repeated 1440p one report the same framebuffer. Confirmed on a DE10 with a 4K monitor.
- Keep Full display's text at much the same size on every display. It used to take the largest canvas that divides the framebuffer exactly, so 720p and 1440p got 640x360 and text three quarters of 1080p's size or less; it now picks the canvas whose height lands nearest 270 rows: 426x240 on 720p and 1440p, 455x256 on 1366x768, 533x300 on 1600x900, 420x262 on 1680x1050. 1080p, 4K and every CRT mode keep their canvas. On 720p, the most common HDMI mode, the list now opens with fewer, larger rows.
- Credits lists gambleboyen among the early adopters.
- Refresh the embedded first-run catalogue snapshot (1276 rows, 2026-09-22).

## v1.1.1 — 2026-09-21

- Update MisterZine alone, without Update All: when the catalogue check finds a newer release, Options gains "Update MisterZine only" above the Update All row, which runs Downloader for the misterzine database by itself under the same supervisor, screen and restart offer. Update All brings the whole card along and can take many minutes for a two-megabyte app; this takes seconds and leaves everything else as it was. The run finds Downloader where Update All keeps it, under `Scripts/.config/downloader`, taking the card's launcher script when it has one and otherwise running that copy with the environment the script would have exported; a card with neither no longer shows the row at all, rather than offering a button that cannot work. A run carries the mode it was started in, so a MisterZine update no longer calls itself Update All.
- List a game the catalogue knows only through cores this card has not got when a file on the card does play it. Such a game's row is greyed and refuses to launch, and the card's own copy — another author's core, or one from a database the tracker does not follow — fell between that row and the rule that a game the catalogue names is never listed twice, so it was nowhere. It now appears beside the greyed row as a local game whose Details say the catalogue lists it only for cores that are not on this card. The core index decides it, so the row goes when the catalogue's own core arrives, handing over its favorite, chosen version and launch history.
- Turn the display from the list: holding Select and pressing Left or Right steps through monitor CW, horizontal and monitor CCW and saves the turn, so a screen that came up the wrong way round is righted where you notice it instead of three screens into Options. The held-Select legend gains the arrows and the notice names the new rotation, and says the once that the turn switched Follow INI rotation off.
- Reopen on the launched game whatever view starts. A launch now records the list order it was made from, and the return picks that view when the one the app opens in does not list the game: with Remember last view off the app starts in the Default view, which need not hold the game, and the row was left at the top with no explanation. A game the current filters hide is still not shown, and the log says so.
- Play the cabinet animation before a game only when Start is held: an unset Launch transition now means Hold Start, so a tap launches at once. Always stays the other choice and a saved preference is untouched.
- Credits lists the Patreon supporters, current and past, and reads the early adopters from the site file the app already fetches after each catalogue check, so a new name reaches every online device on its next check rather than waiting for a release. A device that has never been online keeps the embedded snapshot's list.
- Add kuzecores (kuzearcade) as a source, and let a new site source wire itself: the generator imports the site's feature sections and opt-in databases, the app derives its database table from them, a short chip comes from the display name when no hand-picked one exists, and CI fails when the generated tables are behind the public site.
- Document MiSTer's native Y/C output in Troubleshooting: the framebuffer MisterZine draws into reaches the analog pins only through the scaler path, so no MiSTer.ini setting shows it over S-Video or composite. A `[MisterZine]` section with `direct_video=1` gives it an RGB output while game cores keep Y/C; on a CRT through a Y/C adapter that shows MisterZine in black and white, confirmed on hardware.
- Open the README with the tour playing in the page and eleven screenshots reshot against the current app, aligned to the top of each gallery row.
- Refresh the embedded first-run catalogue snapshot (1274 rows, 2026-09-21).

## v1.1.0 — 2026-09-19

- Add local games: the card scan reads every `.mra` under `_Arcade` (except `_Organized`, and skipping BIOS files) that no catalogue release matches, builds a row from the MRA header (title, setname, year, manufacturer, core, ROM archive) and merges it into the dataset as a "Local" source with a "local" chip, deduplicated by setname. Details carries a note and the File line; the Filters Source section counts them; the scan log reports how many MRAs sit outside the catalogue's folders. Local rows carry no dates and stay out of the seen and since-visit bookkeeping, and the dataset's generation id is separate from the catalogue hash so a local change never looks like a catalogue update. Screenshots for local games come from images.misterzine.fyi, with remembered misses expiring. When the catalogue later lists a local game, its favorite, chosen version and recents move to the catalogue row. Local games join the screensaver's screenshot pool.
- Refuse to launch a game whose core is not on the card. Start on a greyed row whose MRA exists but whose core is missing handed the file to MiSTer, which sat on a load that never completed until the watcher gave up after 20 seconds. Such a game and its alternatives are now marked not launchable and the notice names the core.
- Play a launch animation before a game starts: a low-poly arcade cabinet loaded from an OBJ, with the game's screenshot texture-mapped onto its curved screen, approaches with a forward lean, turns once and fades to black; the screen stays black while the core loads. Frames render ahead on their own goroutine and are pushed one per vertical blank, holding 60 fps on the boards; the clock only guards against stalls. Vertical shots keep 3:4. Options -> Display -> Launch transition chooses Always or Hold Start (a tap launches at once, a held Start plays it) and A previews it from the row. A button released during the animation is no longer treated as held afterwards, and the screensaver waits while the animation and the core own the screen. Reopening after a game centres the page on the game that was launched.
- Add the Exit chord: Select+Start, or L+R+Select+Start, held one second inside a launched game leaves it for the MiSTer menu, as Reset in the OSD would, and Return after game brings MisterZine back. Off by default under Options -> Operation. The launcher polls the pads' key state through EVIOCGKEY, which Main's exclusive grab does not hide.
- Replace the in-list "last look" divider with a since-visit status row: the first line of every view counts what was added or updated since the previous visit over the enabled catalogue ("2 added, 1 updated since your last visit, yesterday", or that nothing was), and every row added or rebuilt since then shows a green date wherever it sorts. The divider only worked under the updated sort, and a build catalogued late under an old date pushed it below unchanged rows, which then read as news. The Updated view is named Build date. Mirrors the site's 2026-09-17 change.
- Mark Coin-Op Collection's Patreon-gated cores like Jotego's: the chip reads alpha or beta, and the Details line follows the feed's gate term (Jotego's jtbeta.zip, a Coin-Op licence key, or a generic key line). The user guide covers both sources.
- Add Slop Cores (TheJesusFish) to the source tables: site name, Slop chip, and an opt-in database for Installed only mode. A test now demands all three for every site source.
- Move the Options build, catalogue and Last checked lines from a fixed footer to greyed info rows after Quit MisterZine, so the list gains their room and the help box sits at the bottom of the body. They are not cursor stops. A catalogue check finishing while Options is open rebuilds the panel.
- Leave the Select+X art type chord out of the Text layout, which has no picture: X leaves the chord legend there and the press does nothing. Options keeps the row live, since the choice applies to the layouts you switch back to, and its hint names the exception.
- License the catalogue data under CC BY 4.0 terms plus a source neutrality condition (LICENSE-CATALOGUE), clarified for setup flows, disparagement and scope.
- Harness: `-local` scenarios with golden renders for local rows, `-launcher` starts with the main menu shortcut on so Open at boot, Return after game and Exit chord render live, and a launch cabinet preview script and frame benchmark. Seven stray root renders a mis-flagged run left in `out/` are dropped from the golden manifest.
- Refresh the embedded first-run catalogue snapshot (1242 rows, 2026-09-19).
